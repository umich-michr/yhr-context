...
| Publishability | Institutionally governed process | No |
| Participant-facing study content | Application study team | Yes |
| Study eligibility criteria | Application study team | Yes |
| Study activation and deactivation dates | Application study team; backend administrator override | Yes |
...
When an `APP_USER` already exists, later imported name or email changes are not
copied under the current implementation.
