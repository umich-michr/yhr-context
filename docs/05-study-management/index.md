---
title: Study Management
summary: Study posting creation, information authoring, membership, notifications, lifecycle, and archiving.
status: authoritative
---

# Study Management

An operational study posting is the participant-facing recruiting representation of an imported institutional study.

## Posting workflow

A study posting:

- References one imported `study_num`
- Is unique for that `study_num`
- Begins inactive
- Contains participant-facing study information
- Defines participant type
- May define eligibility criteria
- May contain one screening questionnaire
- Is governed by institutional publishability
- Is activated and deactivated through study-team actions and system rules

## Study-management pages

- [Posting Creation](posting-creation.md)
- [Study Information Authoring](study-information-authoring.md)
- [AI-Assisted Study Posting Authoring](ai-assisted-posting-authoring.md)
- [Study Lifecycle](study-lifecycle.md)
- [Study Archiving](study-archiving.md)
- [Study Notifications](study-notifications.md)

## Related pages

- [Eligibility-Criteria Authoring](../06-recruitment/eligibility-criteria-authoring.md)
- [Questionnaires and Exports](../06-recruitment/questionnaires-and-exports.md)
- [Study Membership](../04-users-and-access/study-membership.md)
- [Publishability](../03-institutional-governance/publishability.md)
